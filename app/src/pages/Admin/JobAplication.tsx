...
import { Check, X, Clock, Mail, Phone, Calendar, FileText } from "lucide-react";
...
const StatusBadgeMap = {
  InProgress: "bg-yellow-100 text-yellow-800 text-sm font-medium px-3 py-1 rounded-full",
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          {application.status === "In Progress" && (
            <div className="flex gap-3 w-full sm:w-auto">
             <ConfirmModal buttonText={"Reject"} action={()=>handleRejectCandidate(application._id)} alertDescription='  This action will reject this candidate. This cannot be undone.'/>
              <ConfirmModal buttonText={'Select'} action={() => handleSelectCandidate(application._id)} alertDescription='  This action will Select this candidate. '/>
            </div>
          )}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-8 p-8">
        <div className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-gray-50 p-6 rounded-lg border border-gray-100 dark:border-gray-800 dark:text-white dark:bg-gray-900">
              <p className="text-lg leading-relaxed whitespace-pre-wrap">{application.coverLetter}</p>
            </div>
          </div>
        </div>
      </CardContent>
